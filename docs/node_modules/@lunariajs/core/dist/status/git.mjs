...
function getGitHostingLinks(repository) {
  ...
  switch (hosting) {
    case "github":
      return {
        create: (filePath) => `https://github.com/${name}/new/${branch}?filename=${join(rootDir, filePath)}`,
        source: (filePath) => `https://github.com/${name}/blob/${branch}/${join(rootDir, filePath)}`,
        history: (filePath, sinceDate) => `https://github.com/${name}/commits/${branch}/${join(rootDir, filePath)}${sinceDate ? `?since=${sinceDate}` : ""}`,
        clone: () => `https://github.com/${name}.git`
      };
    case "gitlab":
      return {
        create: (filePath) => `https://gitlab.com/${name}/-/new/${branch}?file_name=${join(rootDir, filePath)}`,
        source: (filePath) => `https://gitlab.com/${name}/-/blob/${branch}/${join(rootDir, filePath)}`,
        history: (filePath, sinceDate) => `https://gitlab.com/${name}/-/commits/${branch}/${join(rootDir, filePath)}${sinceDate ? `?since=${sinceDate}` : ""}`,
